...
public class TankApp extends GameApplication {
     ...
     * 顶部的三个点,用于产生敌军坦克
     */
    private static final int[] ENEMY_HOME = {30, 295 + 30, 589 + 20};
    ...
    public void buildAndStartLevel() {
        ...
        Timeline tl = new Timeline(new KeyFrame(Duration.seconds(1.2), new KeyValue(rect1.translateYProperty(), -getAppHeight() / 2.0), new KeyValue(rect2.translateYProperty(), getAppHeight() / 2.0)));
    ...
    private void startLevel() {
        ...
        if (geti("level") == 0) {
            Level level;
            try {
                level = new TMXLevelLoader().load(new File(GameConfig.CUSTOM_LEVEL_PATH).toURI().toURL(), getGameWorld());
        ...
        // 首先产生几个敌方坦克
        for (int j : ENEMY_HOME) {
            spawn("enemy", new SpawnData(j, 30).put("assentName", "tank/E" + FXGLMath.random(1, 12) + "U.png"));
            inc("spawnedEnemy", 1);
        }
        spawnEnemy();
    }
        ...
        spawnEnemyTimerAction = run(() -> {
            ...
            for (int i = 0; i < testTimes; i++) {
                if (geti("spawnedEnemy") < GameConfig.ENEMY_AMOUNT) {
                    ...
                    // 随机抽取数组的一个x坐标
                    int x = ENEMY_HOME[new Random().nextInt(3)];
                    int y = 30;
                    ...
                    if (canGenerate) {
                        inc("spawnedEnemy", 1);
                        spawn("enemy", new SpawnData(x, y).put("assentName", "tank/E" + FXGLMath.random(1, 12) + "U.png"));
                    }
                    // 隐藏这个实体
